window.onload = () => {
    ...
    // 태그 분류
    const predictTagBtn = document.querySelector("[data-ai-predict-tag]");
    if (predictTagBtn) {
        const tagListBox = document.querySelector("[data-ai-tag-list]");
        const tagMessageBox = document.querySelector("[data-ai-tag-message]");
        const titleInput = document.querySelector("[data-form-field='adTitle']");
        const bodyInput = document.querySelector("[data-form-field='adBody']");

        predictTagBtn.addEventListener("click", async (e) => {
            const title = titleInput.value.trim();
            const description = bodyInput.value.trim();
            if (!title && !description) return;

            tagListBox.replaceChildren();
            tagMessageBox.textContent = "";

            advertisementService.predictTag(title, description, (result) => {
                console.log(result);

                if (result.tags.length === 0) {
                    tagMessageBox.textContent = "추천할 태그를 찾지 못한걸 보니 새로운 유형의 광고네요!";
                    return;
                }

                result.tags.forEach(tag => {
                    const chip = document.createElement("span");
                    chip.className = "MarketplaceAdTagChip";
                    chip.textContent = `#${tag}`;
                    tagListBox.appendChild(chip);
                });
            });
        });
    }
