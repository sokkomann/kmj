document.addEventListener('DOMContentLoaded', () => {
    ...
    const chatStartToggle = document.getElementById('vaiChatStartToggle');
    ...
    // 11-2. 대화 시작 버튼 클릭시 채팅 패널 열기
    chatStartToggle.addEventListener('click', function (e) {
        e.stopPropagation();
        currentMeeting = null;
        chatTitle.textContent = 'AI에게 질문';
        resetChat();
        dropdown.classList.remove('open');
        closeAllPanels();
    ...
    function resetChat() {
        chatMessages.innerHTML =
            ...
            '  </svg>' +
            '  <span>최근 작성된 게시글에 대해 질문해보세요</span>' +
            '</div>';
    }
    ...
    async function sendMessage() {
        ...
        chatTextarea.value = "";
        console.log("사용자 메세지 들어옴")
        chatTextarea.style.height = "auto";

        addMessage("...", "ai")
        const result = await aiService.conversation(text);
        chatMessages.lastElementChild.textContent = result.answer;
        console.log("ai 메세지 들어옴")
    }
